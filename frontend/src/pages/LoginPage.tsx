...
export default function LoginPage() {
  ...
  return <main className="auth-page">
    ...
    <section className="auth-panel"><form className="auth-card" onSubmit={submit}><span className="eyebrow">{registerMode ? "Bắt đầu hành trình" : "Chào mừng trở lại"}</span><h2>{registerMode ? "Tạo tài khoản" : "Đăng nhập"}</h2><p>{registerMode ? "Thiết lập hồ sơ dinh dưỡng chỉ trong vài phút." : "Tiếp tục kế hoạch ăn uống của bạn."}</p>
      ...
      <label>Mật khẩu<input required type="password" minLength={8} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="Tối thiểu 8 ký tự" /></label>
